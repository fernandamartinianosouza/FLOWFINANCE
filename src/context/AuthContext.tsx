...
const normalizarPerfil = (valor: unknown): PerfilUsuario | null => {
  if (
    typeof valor === 'string' &&
    PERFIS_VALIDOS.includes(valor as PerfilUsuario)
  ) {
...
const validarSenhaForte = (senha: string) => {
  ...
  if (!/[a-z]/.test(senha)) {
    erros.push('inclua ao menos uma letra minúscula');
  }

  if (!/[A-Z]/.test(senha)) {
    erros.push('inclua ao menos uma letra maiúscula');
  }

  if (!/\d/.test(senha)) {
    erros.push('inclua ao menos um número');
  }
...
export const AuthProvider: React.FC<{
  ...
  const [organizacaoId, setOrganizacaoId] =
    useState<string | null>(null);

  const [vinculoAtivo, setVinculoAtivo] =
    useState(false);
  ...
  /*
   * Mantemos o ID do usuário atual em uma ref.
   *
   * Isso evita tratar eventos repetidos de SIGNED_IN,
   * que o Supabase pode emitir quando a aba volta ao foco,
   * como se fossem um novo login.
   */
  const usuarioAtualIdRef =
  ...
  const carregarAcesso = useCallback(
    async (usuario: User | null) => {
      ...
      if (vinculoResult.error) {
        console.error(
          'Erro ao carregar vínculo do usuário:',
    ...
    const {
      ...
      supabase.auth.onAuthStateChange(
        (
          ...
          /*
           * PASSWORD_RECOVERY continua funcionando
           * normalmente.
           */
          if (
            event ===
          ...
          /*
           * INITIAL_SESSION já é tratada por
           * carregarSessao() acima.
           *
           * Ignorar aqui evita uma segunda carga
           * desnecessária ao abrir o sistema.
           */
          if (
            event ===
            'INITIAL_SESSION'
           ...
           * quando o Supabase renova o token.
           *
           * Isso NÃO deve colocar o sistema inteiro
           * em loading.
           */
          if (
            event ===
            ...
            /*
             * Só altera o objeto user se for
             * realmente outro usuário.
             *
             * Assim o FinanceContext não entende
             * uma simples renovação de token como
             * mudança de login.
             */
            setUser(
              usuarioAnterior => {
          ...
          /*
           * O Supabase pode emitir SIGNED_IN
           * novamente quando uma aba recupera foco.
           *
           * Se é o MESMO usuário, apenas atualizamos
           * a sessão silenciosamente.
           *
           * NÃO ativamos loading.
           * NÃO recarregamos perfil.
           * NÃO recarregamos o FinanceContext.
           */
          if (
            event ===
            'SIGNED_IN' &&
          ...
          /*
           * USER_UPDATED é um evento legítimo
           * (ex.: alteração de senha/metadados).
           *
           * Atualizamos silenciosamente, sem trocar
           * a aplicação por "Carregando FlowFinance".
           */
          if (
            event ===
              'USER_UPDATED' &&
           ...
           * como um SIGNED_IN de OUTRO usuário,
           * ainda passam pelo fluxo completo.
           */
          window.setTimeout(
            () => {
              if (!ativo) {
  ...
  const signIn = useCallback(
    async (
      ...
      if (
        ...
        throw new Error(
          'Seu usuário não possui acesso ativo a uma organização. Procure o administrador.'
        );
      }
  ...
  const signUp = useCallback(
    async (
      ...
      throw new Error(
        'O cadastro público está desativado. Solicite um convite ao administrador.'
      );
    },
    []
  );
  ...
  const atualizarSenha =
    useCallback(
      async (
        ...
        if (signOutError) {
          console.error(
            'Não foi possível encerrar as demais sessões:',
